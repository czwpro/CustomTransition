//
//  PictureBrowsePopAnimator.swift
...
class PictureBrowsePopAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        UIView.animate(withDuration: transitionDuration(using: transitionContext), delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.3, options: .curveLinear) {
            ...
            if imageFrame.size.width == 0 && imageFrame.size.height == 0 {
                ...
                imageFrame = CGRect(x: (_screenWidth - defaultWidth) * 0.5, y: (_screenHeight - defaultWidth) * 0.5, width: defaultWidth, height: defaultWidth)
            }
