...
class PictureBrowserCell: UICollectionViewCell {
    ...
    weak var delegate: PictureBrowserCellDelegate?
    
    lazy var pictureImageScrollView: BrowseZoomScrollView = {
        
        let pictureImageScrollView = BrowseZoomScrollView(frame: CGRect(x: 0, y: 0, width: _screenWidth, height: _screenHeight))
    ...
    func showWithModel(model: PictureBrowseSouceModel) {
        self.model = model
